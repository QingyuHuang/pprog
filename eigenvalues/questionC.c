...
void findMaxOffDiagonalElement(double *matrix, int dimension, int *row, int *col, int *rowMaxElementIndex);
...
void jacobi(double *matrix, int dimension, double *eigenvalue, double *eigenvector)
{
    ...
    int *rowMaxElementIndex = (int *)malloc(sizeof(int)*dimension);
    memset(rowMaxElementIndex, 0, sizeof(int)*dimension);
    for(i=0; i<dimension; i++)
    {
        double tmpMax = DBL_MIN;
        for(j=i+1; j<dimension; j++)
        {
            if(fabs(matrix[i*dimension+j]) > tmpMax)
            {
                tmpMax = fabs(matrix[i*dimension+j]);
                rowMaxElementIndex[i] = j;
            }
        }
    }
    for(i=0; i<dimension; i++)
    ...
    while(!finishing)
    {
        findMaxOffDiagonalElement(matrixTmp, dimension, &row, &col, rowMaxElementIndex);
        ...
        {
            double tmpMax = DBL_MIN;
            if(fabs(matrixTmp[row*dimension+i]) > tmpMax)
            {
                tmpMax = fabs(matrixTmp[row*dimension+i]);
                rowMaxElementIndex[row] = i;
            }
        }
        for(i=col+1; i<dimension; i++)
        {
            double tmpMax = DBL_MIN;
            if(fabs(matrixTmp[col*dimension+i]) > tmpMax)
            {
                tmpMax = fabs(matrixTmp[col*dimension+i]);
                rowMaxElementIndex[col] = i;
            }
        }
...
void findMaxOffDiagonalElement(double *matrix, int dimension, int *row, int *col, int *rowMaxElementIndex)
{
    ...
    for(i=0; i<dimension; i++)
    {
        if(fabs(matrix[i*dimension+rowMaxElementIndex[i]]) > minElement)
        {
            minElement = fabs(matrix[i*dimension+rowMaxElementIndex[i]]);
            *row = i;
            *col = rowMaxElementIndex[i];
        }
    }
    return;
}
